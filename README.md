...
- 🎯 Entry points to execute your program using `python -m <ml_backend>` or `$ ml_backend` with basic CLI argument parsing.
...
[![codecov](https://codecov.io/gh/xbkaishui/ml_backend/branch/main/graph/badge.svg?token=ml_backend_token_here)](https://codecov.io/gh/xbkaishui/ml_backend)
[![CI](https://github.com/xbkaishui/ml_backend/actions/workflows/main.yml/badge.svg)](https://github.com/xbkaishui/ml_backend/actions/workflows/main.yml)

Awesome ml_backend created by xbkaishui

## Install

from source
```bash
git clone https://github.com/xbkaishui/ml_backend ml_backend
cd ml_backend
make install
```
...
```bash
pip install ml_backend
```

## Executing

```bash
$ ml_backend run --port 8080
```

or

```bash
python -m ml_backend run --port 8080
```

or

```bash
$ uvicorn ml_backend:app
```
...
```bash
❯ ml_backend --help
Usage: ml_backend [OPTIONS] COMMAND [ARGS]...
...
```bash
❯ ml_backend create-user --help
Usage: ml_backend create-user [OPTIONS] USERNAME PASSWORD
...
```bash
ml_backend create-user admin admin --superuser
```
...
Run with `ml_backend run` and access http://127.0.0.1:8000/docs

![](https://raw.githubusercontent.com/rochacbruno/fastapi-project-template/master/docs/api.png)


**For some api calls you must authenticate** using the user created with `ml_backend create-user`.
...
-----------------------------------------------------
ml_backend/__init__.py              4      0   100%
ml_backend/app.py                  16      1    94%
ml_backend/cli.py                  21      0   100%
ml_backend/config.py                5      0   100%
ml_backend/db.py                   10      0   100%
ml_backend/models/__init__.py       0      0   100%
ml_backend/models/content.py       47      1    98%
ml_backend/routes/__init__.py      11      0   100%
ml_backend/routes/content.py       52     25    52%
ml_backend/routes/security.py      15      1    93%
ml_backend/routes/user.py          52     26    50%
ml_backend/security.py            103     12    88%
-----------------------------------------------------
TOTAL                               336     66    80%
...
> `dynaconf_merge` is a boolean that tells if the settings should be merged with the default settings defined in ml_backend/default.toml.
